...
VALIDATION_SCRIPT_PATH="/tmp/validation-2_11_1.py"
...
function validate_security_group() {
  ...
  aws s3 cp s3://wandisco-public-files/scripts/validation-2_11_1.py "$VALIDATION_SCRIPT_PATH"
  if [[ ! -e  "$VALIDATION_SCRIPT_PATH" ]]; then
    error_exit "Could not download validation-2_11_1.py." "$FUSION_INSTALLED_HANDLER" 1
  else
